...
Application::Application() : dataFilename("vfs_data.txt") { initialize(); }

Application::Application(const std::string &dataFilename)
    : dataFilename(dataFilename) {
  initialize();
}

void Application::initialize() {
  auto persistedFileSystem =
      std::make_unique<FileSystemPersistence>(dataFilename);
...
void Application::run() { processCommands(); }
