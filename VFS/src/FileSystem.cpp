...
bool FileSystem::changeDir(const std::string &path) {
  ...
  if (!path.empty() && path.front() == '/') {
    ...
    currentPathComponents.insert(currentPathComponents.end(), comps.begin(),
                                 comps.end());
  } else {
...
FileSystem::getFileSystemComponent(const std::string &path) const {
  if (path == "/" || path.empty()) {
    return root;
  }
...
FileSystem::getDirectory(const std::string &path) const {
  auto obj = getFileSystemComponent(path);
  if (obj && obj->isDirectory()) {
...
FileSystem::listCurrentDirectory() const {
...
bool FileSystem::appendToFile(const std::string &path,
                              const std::string &content) {
  auto file = getFile(path);
  if (file) {
...
FileSystem::splitPathToComponents(const std::string &path) const {
...
std::string
FileSystem::joinPath(const std::vector<std::string> &pathComponents) const {
  if (pathComponents.empty() ||
      (pathComponents.size() == 1 && pathComponents[0] == "/")) {
    return "/";
  }
...
FileSystem::resolveParentDirectory(const std::string &path,
  ...
  if (isAbsolute) {
    ...
    absolutePath.insert(absolutePath.end(), pathComponents.begin(),
                        pathComponents.end());
