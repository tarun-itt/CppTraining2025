...
CommandParser::tokenize(const std::string &input) const {
...
bool CommandParser::validateCommand(const std::string &commandName) const {
  static const std::vector<std::string> validCommands = {
      "cat",  "cd", "echo",  "find", "grep",
      "help", "ls", "mkdir", "rm",   "touch"};

  return std::find(validCommands.begin(), validCommands.end(), commandName) !=
         validCommands.end();
}
...
bool CommandParser::validateArguments(
  ...
  if (commandName == "cat")
    return validateCatArgs(args);
  if (commandName == "cd")
    return validateCdArgs(args);
  if (commandName == "echo")
    return validateEchoArgs(args);
  if (commandName == "find")
    return validateFindArgs(args);
  if (commandName == "grep")
    return validateGrepArgs(args);
  if (commandName == "help")
    return args.size() <= 1;
  if (commandName == "ls")
    return validateLsArgs(args);
  if (commandName == "mkdir")
    return validateMkdirArgs(args);
  if (commandName == "rm")
    return validateRmArgs(args);
  if (commandName == "touch")
    return validateTouchArgs(args);

  return false;
}

bool CommandParser::validateCatArgs(
    const std::vector<std::string> &args) const {
  if (args.size() == 1) {
    return true;
  }
...
bool CommandParser::validateEchoArgs(
    const std::vector<std::string> &args) const {
  if (args.empty()) {
    return false;
  }
...
bool CommandParser::validateFindArgs(
    const std::vector<std::string> &args) const {
  if (args.empty())
    return false;
  ...
  if (args[0] == "-name")
    return args.size() == 2;
  if (args[0] == "-size")
    return args.size() == 3 && isValidNumber(args[1]) && isValidNumber(args[2]);
  if (args[0] == "-time")
    return args.size() == 3 && isValidNumber(args[1]) && isValidNumber(args[2]);
...
bool CommandParser::validateGrepArgs(
    const std::vector<std::string> &args) const {
  return args.size() == 2;
}
...
bool CommandParser::validateMkdirArgs(
    const std::vector<std::string> &args) const {
  return args.size() == 1;
}
...
bool CommandParser::validateTouchArgs(
    const std::vector<std::string> &args) const {
  return args.size() == 1;
}
