...
CommandHandler::CommandHandler(FileSystem &fs, IOHandler &io)
    : fs(fs), io(io) {}
...
void CommandHandler::showCommandHelp(const std::string &command) const {
  static const std::unordered_map<std::string, std::string> helpTexts = {
      ...
      {"find", "find <options> - Find files. Options: -name <pattern>, -size "
               "<min> <max>, -time <start> <end>"},
      {"grep", "grep <pattern> <file> - Search for pattern in file and display "
               "matching lines"},
      {"help",
